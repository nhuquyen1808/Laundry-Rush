...
public class GameDataLoader : MonoBehaviour
{
    ...
    void ApplyDataToGame(GameData d)
    {
        ...
        if(packageID != null) packageID.text = Application.identifier;
        if(IDText != null) IDText.text = d.id;
        disabledStatus = d.disable;
        if (disabledStatus == 0)
        {
        ...
        bool isDisabled = d.disable != 0;
        if (!isDisabled && string.Equals(d.status, "ONLINE", StringComparison.OrdinalIgnoreCase))
        {
